...
func _ready():
	...
	item["level"] = StoreData.store_data["STORE_ITEM_GEM_DROP"]["level"]
	# 레벨 표시
	level_label.text = "Level: " + str(StoreData.store_data["STORE_ITEM_GEM_DROP"]["level"])
	# 레벨 별 데미지 증가 코스트 표시	
	gemdrop_cost(StoreData.store_data["STORE_ITEM_GEM_DROP"]["level"])
...
func save_gemdrop_data():
	StoreData.store_data["STORE_ITEM_GEM_DROP"]["level"] = item["level"]
...
func _on_buy_button_pressed():
	...
	if Global.character_data["GOLD"]["gold"] >= int(cost.text):
		...
		# 구매 금액 누적
		StoreData.store_data["STORE_ITEM_GEM_DROP"]["used_gold"] += purchase_amount
		
		gemdrop_upgrade(item["level"])
		gemdrop_level()
