...
func _ready():
	...
	item["level"] = StoreData.store_data["STORE_ITEM_HEALTH"]["level"]
	# 레벨 표시
...
func health_upgrade(item_level):
	match item_level:
		"0":
...
func save_health_data():
	StoreData.store_data["STORE_ITEM_HEALTH"]["level"] = item["level"]
...
func _on_buy_button_pressed():
	...
	if Global.character_data["GOLD"]["gold"] >= int(cost.text):
		...
		# 구매 금액 누적
		StoreData.store_data["STORE_ITEM_HEALTH"]["used_gold"] += purchase_amount
		
		health_upgrade(item["level"])
		health_level()
