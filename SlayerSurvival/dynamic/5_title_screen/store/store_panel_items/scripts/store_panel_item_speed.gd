extends Panel

@onready var name_label        = $Name_label
@onready var level_label       = $Level_label
@onready var description_label = $Description_label
@onready var cost              = $Cost/Cost_label

var item = {
	"name": "name",
	"level":  "0",
	"description": "description",
	"cost": "100",
	"icen_path": "",
	"max_level": 5
}

# Called when the node enters the scene tree for the first time.
func _ready():
	# StoreData에서 현재 레벨을 item에 동기화
	item["level"] = StoreData.store_data["STORE_ITEM_SPEED"]["level"]
	# 레벨 표시
	level_label.text = "Level: " + str(StoreData.store_data["STORE_ITEM_SPEED"]["level"])
	# 레벨 별 데미지 증가 코스트 표시	
	speed_cost(StoreData.store_data["STORE_ITEM_SPEED"]["level"])
	
	# Connect to store reset signal
	get_parent().get_parent().get_parent().get_parent().get_parent().connect("reset_store", Callable(self, "_on_reset_store"))

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(_delta):
	pass

func speed_upgrade(item_level):
	match item_level:
		"0":
			Global.character_data["CHARACTER_STORE_UPGRADES"]["speed"] += 5
		"1":
			Global.character_data["CHARACTER_STORE_UPGRADES"]["speed"] += 5
		"2":
			Global.character_data["CHARACTER_STORE_UPGRADES"]["speed"] += 5
		"3":
			Global.character_data["CHARACTER_STORE_UPGRADES"]["speed"] += 5
		"4":
			Global.character_data["CHARACTER_STORE_UPGRADES"]["speed"] += 5
		"Max":
			pass

func speed_cost(item_level):
	match item_level:
		"0":
			cost.text = "1000"
		"1":
			cost.text = "2000"
		"2":
			cost.text = "3000"
		"3":
			cost.text = "4000"
		"4":
			cost.text = "5000"
		"Max":
			cost.text = "Max"

func speed_level():
	match item["level"]:
		"0":
			item["level"] = "1"
		"1":
			item["level"] = "2"
		"2":
			item["level"] = "3"
		"3":
			item["level"] = "4"
		"4":
			item["level"] = "Max"
		"Max":
			pass

func save_speed_data():
	StoreData.store_data["STORE_ITEM_SPEED"]["level"] = item["level"]
	StoreData.save_store_data()

func _on_buy_button_pressed():
	$Coin_sound.play()
	if Global.character_data["GOLD"]["gold"] >= int(cost.text):
		
		var purchase_amount = int(cost.text)
		Global.character_data["GOLD"]["gold"] -= purchase_amount
		Global.emit_signal("purchase")
		
		# 구매 금액 누적
		StoreData.store_data["STORE_ITEM_SPEED"]["used_gold"] += purchase_amount
		
		speed_upgrade(item["level"])
		speed_level()
		
		save_speed_data()
		Global.save_character_data()
		
		level_label.text = "Level: " + str(item["level"])
		speed_cost(item["level"])

func _on_reset_store():
	item["level"] = "0"
	level_label.text = "Level: " + str(item["level"])
	speed_cost(item["level"])
