...
func _ready():
	...
	item["level"] = StoreData.store_data["STORE_ITEM_SHIELD"]["level"]
	# 레벨 표시
	level_label.text = "Level: " + str(StoreData.store_data["STORE_ITEM_SHIELD"]["level"])
	# 레벨 별 데미지 증가 코스트 표시	
	shield_cost(StoreData.store_data["STORE_ITEM_SHIELD"]["level"])
...
func save_shield_data():
	StoreData.store_data["STORE_ITEM_SHIELD"]["level"] = item["level"]
